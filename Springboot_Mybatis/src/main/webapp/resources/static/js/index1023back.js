$(function(){
			...
			for(var i = 0; i < data.root.length; i++){
				...
				if((typeof data.root[i].assigneeInfo) =="object"){
					if(data.root[i].assigneeInfo.length>0){
						name = data.root[i].assigneeInfo[0].assignee?data.root[i].assigneeInfo[0].assignee.name:"代签收";
	...
	// 				map.deptName = data.rows[i].adepartName;
   ...
   var zdData2 = [{
    ...
    /*$.ajax({
        ...
        success: function (res) {
    		...
    		for(var i = 0; i < data.rows.length; i++){
    			var map = {};
    			if(data.rows[i].status == '开机'){
    				map.pzjgName = data.rows[i].adepartName;
    				map.zcbh = data.rows[i].IP;
    				list.push(map);
    			}
    		}
    		console.log('jigouList---->>>',list);
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            for(var i = 0; i < data.length; i++){
                ...
                map.bjTime = data[i].fLastTime.substr(0,9);
    ...
    $.ajax({
        url: window.location.origin+"/SjwProject/jump/getUrlAssetRiskinfo",
        data: {
            start:0,
            size:10
        },
        success: function (res) {
            ...
            for(var i = 0; i < data.root.length; i++){
                ...
                map.bjInfo = data.root[i].score;
                if(data.root[i].score > 0){
                    map.type = '1';
